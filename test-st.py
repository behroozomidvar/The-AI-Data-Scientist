...
with st.container():
    user_input = st.text_input("Define your data science task", key="user_input")
...
if proceed_button:
    ...
    if st.session_state.next_step_id < len(st.session_state.plan_steps):
        ...
        orchestrator_message = f"**:orange[Orchestrator Agent]**: In step {st.session_state.next_step_id + 1} of {len(st.session_state.plan_steps)}, I do the following: {st.session_state.next_step}"
